...
public class DocumentTypeServiceImpl implements DocumentTypeService {
    ...
    public DocumentTypeResponse getDocumentTypeById(Integer id) {
        ...
        return documentTypeRepository.findById(id)
                .map(DocumentTypeMapper::modelToDocumentTypeResponse)
    ...
    public DocumentTypeResponse createDocumentType(CreateDocumentTypeRequest request) {
        ...
        if (Objects.isNull(request.getCode()) || request.getCode().isBlank()) {
            throw new BadRequestException("El campo code no puede ser nulo.");
        }
        ...
        if (documentTypeRepository.existsByCode(request.getCode())) {
            throw new InternalServerErrorException("Ya existe un tipo de documento con ese code.");
        }
        if (documentTypeRepository.existsByName(request.getName())) {
            throw new InternalServerErrorException("Ya existe un tipo de documento con ese name.");
        }
        try {
            ...
            throw new InternalServerErrorException("Error al guardar el tipo de documento: " + e.getMessage());
        }
    }

    @Override
    public DocumentTypeResponse updateDocumentType(Integer id, CreateDocumentTypeRequest request) {
        if (id == null || id <= 0) {
        ...
        try {
            ...
            throw new InternalServerErrorException("Error al actualizar el tipo de documento: " + e.getMessage());
        }
    }
    ...
    public void deleteDocumentType(Integer id) {
        ...
        try {
            ...
            throw new InternalServerErrorException("Error al eliminar el tipo de documento: " + e.getMessage());
        }
    }
}